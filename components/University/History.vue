  ...
  methods: {
    async getUsers() {
      try {
        ...
        const response = await fetch('http://localhost:8000/api/show_school_admin', {
          method: 'GET',
          headers: {
    ...
    // getUserProfileImage(imagePath) {
    //   if (imagePath) {
    //     return `${process.env.BASE_URL}/${imagePath}`;
    //   }
    //   return '/default-profile-image.png'; // Set a default image path if needed
    // },
  }
};
</script>
